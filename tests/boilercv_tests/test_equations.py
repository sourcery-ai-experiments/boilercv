...
# fmt: off
EXPECTED2 = {
    "akiyama_1973": [1.000000, 0.995887, 0.991767, 0.987640, 0.983507, 0.979367, 0.975219, 0.971065, 0.966903, 0.962734],
    ...
    "florschuetz_chao_1965": [1.000000, 0.946807, 0.924774, 0.907868, 0.893615, 0.881058, 0.869705, 0.859266, 0.849549, 0.840423],
    "inaba_et_al_2013": [1.000000, 0.967928, 0.935856, 0.903785, 0.871713, 0.839642, 0.807570, 0.775499, 0.743427, 0.711355],
    "isenberg_sideman_1970": [1.000000, 0.993721, 0.987422, 0.981104, 0.974765, 0.968405, 0.962024, 0.955622, 0.949199, 0.942753],
    ...
    "yuan_et_al_2009": [1.000000, 0.993324, 0.986629, 0.979915, 0.973182, 0.966429, 0.959656, 0.952864, 0.946050, 0.939216],
}
"""Expected results for each correlation.
...
def test_correlations(correlation, expected):
    """Correlations evaluate as expected."""
    result = correlation(**{
        kwd: value
...
def test_symbolic():
    ...
    mod = symbolic.dimensionless_bubble_diameter
    correlation = lambdify(
        expr=(expr := mod.flourschuetz_chao_1965.rhs.subs(mod.SUBS)),
        modules=numpy,
    ...
    assert allclose(result, EXPECTED2["florschuetz_chao_1965"])


def test_correlations2():
    """Equations evaluate as expected."""
    mod = correlations.dimensionless_bubble_diameter
    result = mod.flourschuetz_chao_1965(**{
        kwd: value
        for kwd, value in KWDS.items()
        if kwd in Signature.from_callable(mod.flourschuetz_chao_1965).parameters
    })
    assert allclose(result, EXPECTED2["florschuetz_chao_1965"])
